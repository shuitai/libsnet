...
const char* inet_ntop_v4(const void* src, char* dst, socklen_t size) 
{
	...
	string_base::sprintfn(dst, size, "%d.%d.%d.%d",
...
const char* inet_ntop_v6(const void* src, char* dst, socklen_t size) 
{
	...
	if (runpos[0] == 1 && (maxpos == 5 || (maxpos == 4 && as_shorts[5] == 0xFFFF))) 
	{
		...
		*cursor++ = ':';
		if (maxpos == 4) 
		{
			cursor += string_base::sprintfn(cursor, INET6_ADDRSTRLEN - 2, "ffff:");
		}
	...
	else 
	{
		for (int i = 0; i < run_array_size; ++i) 
		{
			if (runpos[i] == -1) 
			{
				cursor += string_base::sprintfn(cursor, INET6_ADDRSTRLEN - (cursor - dst), "%x", ntohs(as_shorts[i]));
				if (i != 7 && runpos[i + 1] != 1) 
				{
					*cursor++ = ':';
				}
...
int inet_pton_v6(const char* src, void* dst) 
{
		...
		*(readcursor + 2) != 0) 
	{
		...
		if (string_base::strchr(addrstart, ".")) 
		{
			const char* colon = string_base::strchr(addrstart, "::");
			if (colon) 
			{
				uint16_t a_short;
				int bytesread = 0;
